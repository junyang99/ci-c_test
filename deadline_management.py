...
# check if starting date has exceeded current date
def startdate_valid(starting_date):
    # Get the current date
    current_date = datetime.now().date()

    # Parse the starting_date string to datetime object
    starting_date = datetime.strptime(starting_date, '%Y-%m-%d').date()

    # Check if starting_date is after the current_date
...
# Example usage:
# Input the starting_date in the format 'YYYY-MM-DD'
# starting_date_str = input("Enter starting date (YYYY-MM-DD): ")

# result = startdate_valid(starting_date_str)
# print("Is the starting date valid?", result)
...
@app.route('/open_position/create_role', methods=['POST'])
def create_role():
    if request.is_json:
        try:
            ...
            date_error = startdate_valid(starting_date)
            if date_error:
                fields_error['date_error'] = "Starting date has expired"
            ...
            role = Role(role_name=title, role_desc=description,
...
def get_role():
    ...
    for role in roles:
        ...
        open_position = Open_position.query.filter_by(
            Role_Name=role.role_name).all()
                ...
                if 'skills' in data:
                    ...
                    for skill in skills:
                        role_skill = RoleSkill(
                            role_name=title, skill_name=skill)
                        db.session.add(role_skill)
